...
#ifdef __ANDROID__
#define ZOMDROID_DLOPEN_HOOK dlopen
#define ZOMDROID_DLSYM_HOOK dlsym
#define CALL_LOADER_DLOPEN(filename, flags) loader_dlopen((filename), (flags), __builtin_return_address(0))
#define CALL_LOADER_DLSYM(handle, symbol) loader_dlsym((handle), (symbol), __builtin_return_address(0))
#else
#define ZOMDROID_DLOPEN_HOOK zomdroid_linker_dlopen
#define ZOMDROID_DLSYM_HOOK zomdroid_linker_dlsym
#define CALL_LOADER_DLOPEN(filename, flags) loader_dlopen((filename), (flags))
#define CALL_LOADER_DLSYM(handle, symbol) loader_dlsym((handle), (symbol))
#endif
...
#else
static void* (*loader_dlopen)(const char* filename, int flags);
static void* (*loader_dlsym)(void* handle, const char* symbol);
#endif
...
#ifndef __ANDROID__
    if (!loader_dlopen) loader_dlopen = &dlopen;
    if (!loader_dlsym) loader_dlsym = &dlsym;
#endif
...
static char* method_signature_from_symbol_name(const char* sym) {
    ...
    if (g_zomdroid_jvmti_env == NULL || g_zomdroid_main_class_loader == NULL) {
        LOGE("JVMTI/class loader is unavailable; cannot resolve JNI symbol signature for %s", sym);
        return NULL;
    }
...
__attribute__((visibility("default"), used))
void *ZOMDROID_DLOPEN_HOOK(const char* filename, int flags) {
#ifndef __ANDROID__
    if (!loader_dlopen) loader_dlopen = &dlopen;
    if (!loader_dlsym) loader_dlsym = &dlsym;
#endif

    if (filename == NULL) return CALL_LOADER_DLOPEN(NULL, flags);
    ...
    for (int i = 0; i < jni_lib_count; i++) {
        ...
        if (strcmp(jni_libs[i].name, "fmodintegration64") != 0) { //later I should fix that. Java for some reason didn't see classes inside
            ...
            if (access(android_filename, F_OK) == 0) {
                ...
                jni_libs[i].handle = CALL_LOADER_DLOPEN(android_filename, flags);
    ...
    return CALL_LOADER_DLOPEN(filename, flags);
}

__attribute__((visibility("default"), used))
void *ZOMDROID_DLSYM_HOOK(void *handle, const char *sym_name) {
#ifndef __ANDROID__
    if (!loader_dlopen) loader_dlopen = &dlopen;
    if (!loader_dlsym) loader_dlsym = &dlsym;
#endif
    ...
    for (int i = 0; i < jni_lib_count; i++) {
        ...
        if (elib->is_emulated) {
            ...
            return CALL_LOADER_DLSYM(handle, sym_name);
        }
    }
