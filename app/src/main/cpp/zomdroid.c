...
#ifndef __ANDROID__
static void* try_dlopen_jvm(const char* library_dir_path) {
    const char* env_jvm_path = getenv("ZOMDROID_JVM_LIB");
    if (env_jvm_path && env_jvm_path[0]) {
        void* handle = dlopen(env_jvm_path, RTLD_NOW | RTLD_GLOBAL);
        if (handle) {
            LOGI("Loaded JVM from ZOMDROID_JVM_LIB=%s", env_jvm_path);
            return handle;
        }
        LOGW("Failed loading JVM from ZOMDROID_JVM_LIB: %s", dlerror());
    }

    if (library_dir_path && library_dir_path[0]) {
        char candidate[2048];
        const char* suffixes[] = {
        ...
        for (size_t i = 0; i < sizeof(suffixes) / sizeof(suffixes[0]); i++) {
            snprintf(candidate, sizeof(candidate), "%s%s", library_dir_path, suffixes[i]);
            void* handle = dlopen(candidate, RTLD_NOW | RTLD_GLOBAL);
            if (handle) {
                LOGI("Loaded JVM from %s", candidate);
                return handle;
            }
        }
    }

    void* handle = dlopen("libjvm.dylib", RTLD_NOW | RTLD_GLOBAL);
    if (handle) return handle;
    return dlopen("libjvm.so", RTLD_NOW | RTLD_GLOBAL);
}
#endif

static void create_jvm_and_launch_main(const char* library_dir_path, int jvm_argc, const char** jvm_argv, const char* main_class_name, int argc, const char** argv) {
#ifdef __ANDROID__
    void* libjvm = linkernsbypass_namespace_dlopen("libjvm.so", RTLD_GLOBAL, zomdroid_ns);
#else
    void* libjvm = try_dlopen_jvm(library_dir_path);
#endif
    if (libjvm == NULL) {
        LOGE("%s", dlerror());
        return;
    ...
    if (!JNI_CreateJavaVM) {
        LOGE("Failed to locate JNI_CreateJavaVM symbol");
        return;
    }
    ...
    if (jvm_argc > 0) {
        options = calloc((size_t)jvm_argc, sizeof(*options));
        if (!options) {
            LOGE("Failed to allocate JVM options");
            return;
        }
    }
    if (jvm_argv != NULL && options != NULL) {
    ...
    if (res != JNI_OK) {
        ...
        return;
#else
        LOGW("JVM TI is unavailable, continuing without JVMTI (error code: %d)", res);
        jvmtiEnv = NULL;
#endif
    }
...
//    }
    ...
    if (jvmtiEnv != NULL) {
        if ((err = (*jvmtiEnv)->GetClassLoader(jvmtiEnv, main_class, &classLoader)) != JVMTI_ERROR_NONE) {
            LOGW("GetClassLoader() via JVMTI failed, error code %d", err);
        }
    }
    if (classLoader == NULL) {
        ...
        if (class_class != NULL) {
            ...
            if (get_class_loader != NULL) {
                classLoader = (*env)->CallObjectMethod(env, main_class, get_class_loader);
            }
        }
    }
    if (classLoader != NULL) {
        ...
        LOGW("Class loader is unavailable, JNI signature lookup may be limited");
    }
...
static int load_linker_hook() {
    ...
    const char* candidates[] = {
        ...
        "libzomdroidlinker.so"
    };

    void* linker_handle = NULL;
    for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
        if (!candidates[i] || !candidates[i][0]) continue;
        linker_handle = dlopen(candidates[i], RTLD_NOW | RTLD_GLOBAL);
        if (linker_handle) {
            LOGI("Loaded linker hook library: %s", candidates[i]);
            break;
        }
    }

    if (!linker_handle) {
        LOGW("Linker hook library not found. Continuing without box64 linker hook.");
        return 0;
    }
...
void zomdroid_start_game(const char* game_dir_path, const char* library_dir_path, int jvm_argc,
    ...
    create_jvm_and_launch_main(library_dir_path, jvm_argc, jvm_argv, main_class_name, argc, argv);
}
...
void zomdroid_surface_init(ZomdroidNativeWindow* wnd, int width, int height) {
    pthread_mutex_lock(&g_zomdroid_surface.mutex);
